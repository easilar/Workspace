...
path = "/afs/hephy.at/user/e/easilar/www/data/Run2015D/133pb/zeroB_corrected/"+lepSel['label']+"/"
#path = "/afs/hephy.at/user/e/easilar/www/data/Run2015D/133pb/multiB/"+lepSel['label']+"/"
...
#'cut':"&&".join([lepSel['cut'],lepSel['veto'],lepSel['trigger'],ht_cut,st,njets_30_cut,jets_2_80,filters,nbjets_30_cut_zero]),\
'cut':"&&".join([ht_cut,lepSel['cut'],lepSel['veto'],lep_hard,lepSel['trigger'],'nJet>=4',jets_2_80,st,filters,nbjets_30_cut_zero,jets_2_80]),\
...
'bkgcut':"&&".join([ht_cut,lepSel['cut'],lepSel['veto'],lep_hard,"nJet>=4",jets_2_80,st,nbjets_30_cut_zero,jets_2_80]),\
...
data_chain = getChain(samples[0]['chunk'],maxN=maxN,histname="",treeName="tree")
...
plots =[\
{'ndiv':False,'yaxis':'Events','xaxis':'#Delta#Phi(W,l)','logy':'True' , 'var':dPhi,                 'varname':'deltaPhi_Wl',       'binlabel':1,  'bin':(30,0,3.141)},\
...
for p in plots:
  ...
  for bkg in bkg_samples:
    ...
    histo = getPlotFromChain(chain, p['var'], p['bin'], cutString = cut['bkgcut'], weight = "weight*0.80*"+str(lumi)+"/3000", binningIsExplicit=False, addOverFlowBin='both')
    print histo
    #histo.Draw("Bar")
  ...
  latex.DrawLatex(0.6,0.5,"#bf{MC scale=0.80}")
  Pad1.RedrawAxis()
  can.cd()
