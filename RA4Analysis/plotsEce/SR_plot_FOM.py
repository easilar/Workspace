...
deltaPhiCuts = array('d', [float(x)/100. for x in range(10,200,5)]) 
print deltaPhiCuts
...
for srNJet in sorted(signalRegions):
  ...
  for stb in sorted(signalRegions[srNJet]):
    ...
    for htb in sorted(signalRegions[srNJet][stb]):
      ...
      name, c_MB_SR =  nameAndCut(stb, htb, srNJet, btb=(0,0), presel=presel+"&&deltaPhi_Wl>"+str(deltaPhiCut), btagVar = btagString)
      nameb, cutb =  nameAndCut(stb, htb, srNJet, btb=(0,0), presel=presel_bkg+"&&deltaPhi_Wl>"+str(deltaPhiCut), btagVar = btagString)
      dummy, c_MB_CR =  nameAndCut(stb, htb, srNJet, btb=(0,0), presel=presel+"&&deltaPhi_Wl<"+str(deltaPhiCut), btagVar = btagString)
      dummy, c_SB_tt_CR =  nameAndCut(stb, htb, (4,5), btb=(1,1), presel=presel+"&&deltaPhi_Wl<"+str(deltaPhiCut), btagVar = btagString)
      dummy, c_SB_W_CR =  nameAndCut(stb, htb, (3,4), btb=(0,0), presel=presel+"&&deltaPhi_Wl<"+str(deltaPhiCut), btagVar = btagString)
      dummy, c_SB_tt_SR =  nameAndCut(stb, htb, (4,5), btb=(1,1), presel=presel+"&&deltaPhi_Wl>"+str(deltaPhiCut), btagVar = btagString)
      dummy, c_SB_W_SR =  nameAndCut(stb, htb, (3,4), btb=(0,0), presel=presel+"&&deltaPhi_Wl>"+str(deltaPhiCut), btagVar = btagString)
      ...
      for sig in allSignals:
        ...
        for mglu in signal.keys() :
          ...
          for mlsp in signal[mglu].keys() :
            ...
            bin[srNJet][stb][htb]["signals"][mglu][mlsp] = {"yield_MB_SR":getYieldFromChain(s_chain, c_MB_SR, weight = weight_str),"err_MB_SR":sqrt(getYieldFromChain(s_chain, c_MB_SR, weight = weight_str+"*"+weight_str)),\
                                                          "yield_MB_CR":    getYieldFromChain(s_chain, c_MB_CR, weight = weight_str)      ,"err_MB_CR":     getYieldFromChain(s_chain, c_MB_CR, weight    = weight_str+"*"+weight_str)             ,\
                                                          "yield_SB_tt_SR": getYieldFromChain(s_chain, c_SB_tt_SR, weight = weight_str)   ,"err_SB_tt_SR":  getYieldFromChain(s_chain, c_SB_tt_SR, weight = weight_str+"*"+weight_str)          ,\
                                                          "yield_SB_tt_CR": getYieldFromChain(s_chain, c_SB_tt_CR, weight = weight_str)   ,"err_SB_tt_CR":  getYieldFromChain(s_chain, c_SB_tt_CR, weight = weight_str+"*"+weight_str)          ,\
                                                          "yield_SB_W_SR":  getYieldFromChain(s_chain, c_SB_W_SR, weight = weight_str)       ,"err_SB_W_SR":   getYieldFromChain(s_chain, c_SB_W_SR, weight     = weight_str+"*"+weight_str)              ,\
                                                          "yield_SB_W_CR":  getYieldFromChain(s_chain, c_SB_W_CR, weight = weight_str)       ,"err_SB_W_CR":   getYieldFromChain(s_chain, c_SB_W_CR, weight     = weight_str+"*"+weight_str)               \
                                                          }
      bin[srNJet][stb][htb]['name']=name

pickle.dump(bin,file('/data/easilar/Spring15/25ns/allSignals_2p25_pkl','w'))
