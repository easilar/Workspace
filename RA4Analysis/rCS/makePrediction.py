...
cBkg = getChain([WJetsHTToLNu_25ns,TTJets_combined,singleTop_25ns, DY_25ns,TTV_25ns],histname='')#no QCD, no TTVH, no DY
...
for weight_dict in weights:
  ...
  weight_str, weight_err_str = makeWeight(lumi, sampleLumi=sampleLumi, debug=debugReweighting, in_weight = weight_dict['var'])
  ...
  for srNJet in signalRegions:
    ...
    for stb in signalRegions[srNJet]:
      ...
      for htb in signalRegions[srNJet][stb]:
        ...
        makeTTPrediction(weight_str, weight_err_str , rd, samples, htb, stb, srNJet, presel, dPhiCut=deltaPhiCut, btagVarString = btagString, lumi=lumi, printDir=printDir)

        #join W estimation results to dict
        makeWPrediction(weight_str, weight_err_str , rd, samples, htb, stb, srNJet, presel, dPhiCut=deltaPhiCut, btagVarString = btagString, lumi=lumi, printDir=printDir)
